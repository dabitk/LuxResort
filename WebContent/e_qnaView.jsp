			...
			<c:choose>
				...
				<%--article이 null이 아닌 경우 (에러가 발생하지 않음)에는 이하 코드 실행--%>
					<div>
						...
						<TR>
							<th>조회수</th>
							<td><c:out value="${article.getViewcnt()}"/></td>		
						</TR>				
						<TR>
							<th>내용</th>
							<td>
						...
						<span>
							...
							<input type='hidden' name="rootid" value='<c:out value="${article.getRootid()}"/>'/>
							...
							<input type='submit' value='수정' onclick='this.form.action="e_qnaUpdate.jsp";'/>
							<input type='submit' value='댓글' onclick='this.form.action="e_qnaReinsert.jsp";'/>							
						</span>
						</FORM>		
					</div>
				</c:when>
