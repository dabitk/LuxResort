...
<%@ page import="java.util.*,java.io.*, java.text.SimpleDateFormat,java.net.URLEncoder,java.net.URLDecoder,com.resort.qna.*"%>    
<!DOCTYPE html>
<html lang="en">
  ...
  <section class="page-section" id="services">
    ...
    <div class="container">
      <div class="row">
      	...
      	<p>고객님들의 문의사항을 성심성의껏 답변해드리겠습니다.<p>
      	</div>
        <div class="col-lg-12 text-center">        
		<!-- 이곳에 공지사항 게시판을 추가한다. -->
			<c:choose>
				<c:when test="${!list.isEmpty()}">
					<table align=center cellspacing=1 border=1 style="text-align:left;border-left:none;border-right:none;width:100%">
						...
						<TR>
							...
							<c:choose>
							 <c:when test="${fn:contains(myVar,'[Re]')}">
								<TD class="title">
								<a href="e_qnaView.jsp?key=${article.id}" style="padding-left:${article.relevel-1}0px">
									<img id="reTag" src="<%=request.getContextPath()%>/img/arrow.png" width="5%" height="5%">
									<c:out value="${myVar}"/>
								</a>
								</TD>
							</c:when>
							<c:when test="${fn:contains(myVar,'삭제')}">
								<TD class="title" style="padding-left:${article.relevel-1}0px">
									<img id="reTag" src="<%=request.getContextPath()%>/img/arrow.png" width="5%" height="5%">
									<c:out value="${myVar}"/>
								</TD>
							</c:when>
							<c:otherwise>
								<TD class="title"><a href="<%=request.getContextPath()%>/e_qnaView.jsp?key=${article.id}"><c:out value="${article.title}"/></a></TD>
							</c:otherwise>
							</c:choose>
			...
			<a href="<%=request.getContextPath()%>/e_qnaInsert.jsp" class="btn btn-primary btn-lg rippler rippler-inverse">글쓰기</a>		
        </div>
      </div>
    </div>
  </section>
