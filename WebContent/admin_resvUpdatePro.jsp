...
<head>
  <meta charset="utf-8">
  <META http-equiv="refresh" content="5;URL=admin_allview.jsp">
...
<%
	...
	if(loginOK == null){
		...
		errorcode = resvDAO.updateBookingInfo_Admin(room,checkin,processing);
	}

	
	pageContext.setAttribute("errorcode", errorcode); //페이지 컨텍스트에 resv 속성을 지정한다
%>
  <!-- Navigation -->
		...
		<!-- 이곳에 공지사항 게시판을 추가한다. -->
			<c:choose>
				...
				<%--errorcode가 0인 경우 (에러가 발생하지 않음)에는 이하 코드 실행--%>
					<div>
						<h1 align=center><b><%=checkin%>의 예약상태가 변경 되었습니다.</b></h1>
						<p align=center>5초뒤 목록으로 리다이렉트 됩니다.</p>
						<span>
							<a href="admin_allview.jsp"><button type="button">목록</button></a>
						</span>		
					</div>
				</c:when>
				<c:otherwise>
