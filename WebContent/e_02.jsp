    ...
    <div class="container">
      <div class="row">
		...
		<!-- 이곳에 공지사항 게시판을 추가한다. -->
				...
				<c:when test="${!list.isEmpty()}">
					<table align=center cellspacing=1 border=1 style="text-align:left;border-left:none;border-right:none;width:100%">
						...
						<TR>
							...
							<c:otherwise>
								<TD class="title"><a href="e_qnaView.jsp?key=${article.id}"><c:out value="${article.title}"/></a></TD>
							</c:otherwise>
							</c:choose>
			...
			<a href="e_qnaInsert.jsp" class="btn btn-primary btn-lg rippler rippler-inverse">글쓰기</a>		
        </div>
      </div>
    </div>
  </section>
